...
const AkaCoin = () => {
    ...
    const [akaCoinId, setAkaCoinId] = useState('');
    useEffect(()=>{
        fetch('https://morning-atoll-58806.herokuapp.com/get-akacoin')
        .then(res => res.json())
        .then(data => {
            setAkaCoinId(data._id)
        })
    }, [])
    ...
    const onSubmit = data => {
        ...
        if(image){
            ...
            const URL = `https://morning-atoll-58806.herokuapp.com/update-akacoin/${akaCoinId}`
    ...
    return (
        <>
            <div className="akaCoin__dashboard">
                <form className='dashboard__form' onSubmit={handleSubmit(onSubmit)} encType="multipart/form-data">
                    <div className="text__group">
                        <div className="input__group">
                            <input {...register("akaCoinSectionTitle")} className='form-control' type="text" placeholder='Aka Coin Section Title' />
                        </div>
                        <div className="input__group">
                            <input {...register("akaCoinTitle")} type="text" className="form-control" placeholder='Aka Coin Title' />
                        </div>
                        <div className="input__group">
                            <textarea {...register("akaCoinDescription")} className="form-control" cols="30" rows="3" placeholder='Aka Coin Description'></textarea>
                        </div>
                            ...
                            <label className='image__upload--box' htmlFor="heroSubtitle">
                                {
                                    ...
                                    :
                                    <span className="form__label">Set Aka Coin Left Image</span>
